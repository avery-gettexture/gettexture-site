...
**Migration run and verified (Sep 2, 2026):** the founder ran
`scripts/add_waiver_acknowledged_column.sql` in the Supabase SQL editor
(this session has no direct DB/CLI connection, consistent with every
prior schema migration in this project's history). Verified live
afterward, not assumed from the SQL editor's own success message, per
AGENTS.md's live-table-write rule: a fresh service-role read
(`select('slug, waiver_acknowledged_at')`) against three existing rows
returned the column with `null` on all of them, as expected for rows
